...
- [x] **Add gamescope declaratively.** Done in `modules/gaming/gamescope.nix` (`programs.gamescope.enable = true;`). Note: `capSysNice` is deliberately **off** — with it on, the module ships gamescope only as a setcap wrapper under `/run/wrappers/bin` and drops it from systemPackages, which fails inside Lutris's bubblewrap sandbox (`no_new_privs` blocks the cap). With it off, gamescope lands at the GC-stable `/run/current-system/sw/bin/gamescope`. (Also: a new module file must be `git add`ed or the flake won't see it — untracked files are invisible to flake eval.)
- [x] **Switch the Lutris launch prefix off the store path.** `~/.local/share/lutris/games/battlenet-1781209761.yml`, key `system.prefix_command`, now points at the absolute stable `/run/current-system/sw/bin/gamescope` path (not plain `gamescope` — that path is guaranteed reachable inside the sandbox since `/nix` and `/run` are bind-mounted, with no PATH-resolution uncertainty). Flags set to `-W 3440 -H 1440 -r 100 -f --force-grab-cursor --` for native-res fullscreen on the 3440x1440@100 ultrawide (was `-W 1920 -H 1080 -b`, i.e. non-native borderless). Inside WoW pick "Windowed (Fullscreen)", not exclusive fullscreen. Restart Lutris for it to re-read the yml.
- [x] **Clean up test leftovers** in the Wine prefix: removed the `[Software\\Wine\\Explorer\\Desktops]` virtual-desktop key (`"Default"="1600x900"`) from `/home/lansing/Games/battlenet/user.reg` and deleted `user.reg.bak`. The standalone `[Software\\Wine\\Explorer]` parent key was no longer present. Gamescope now owns the windowing, so the Wine virtual desktop is redundant. (Must be done with the prefix's wineserver fully stopped, or the change is overwritten on shutdown.)
