...
in
{
  ...
  flake.modules.homeManager.development = { pkgs, ... }: {
    ...
    programs.vscode = {
      ...
      # `nix-vscode-extensions` overlay registered in the NixOS half
      # above. Names are lowercased `publisher.name`, matching the
      # marketplace URL slug. VSCodium doesn't ship a marketplace client
      # at runtime, but Nix-installed extensions are just unpacked into
      # the extensions dir, so the MS marketplace mirror works regardless
      # (Open VSX would miss several of these, e.g. anthropic.claude-code).
