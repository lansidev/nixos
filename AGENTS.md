...
- **`flake-parts`** — flake outputs are evaluated as Nixpkgs-style modules; provides `perSystem` and (via its `flakeModules.modules` import in `modules/meta/flake.nix`) the `flake.modules.<class>.<name>` namespace the dendritic layout is built on
...
modules/                                   # ALL files below are flake-parts modules, auto-loaded by import-tree
  meta/
    flake.nix                              # enables flake-parts' flakeModules.modules (the flake.modules.* namespace) + declares systems (x86_64-linux + aarch64-darwin for the Mac devShell)
...
- The home-manager halves of `desktop`/`development` reach lansing through the coupling in `modules/users/lansing.nix` — a host that imports `nixos.desktop` automatically gets `homeManager.desktop`. New HM-only files just contribute to the right `flake.modules.homeManager.<bucket>`; no import wiring needed.
