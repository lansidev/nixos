...
- **The main Pi model is temporarily `qwen3-next-80b-a3b-thinking`, not `glm-4.6`.** `glm-4.6` on Cortecs (AtlasCloud backend) returns *truncated* tool-call function names (`ask_user_question` → `ask_u`) — verified against the raw API, stream and non-stream; Qwen models are unaffected — so any tool with a multi-token name breaks under it. (`qwen3-coder-next` fixes the name but serialises *nested* tool arguments as JSON strings, breaking `ask_user_question`/`write`; `qwen3-next-80b-a3b-thinking` is correct on both — `devstral-2512` is too, but it's loop-prone.) `defaultModel` is therefore pinned to `qwen3-next-80b-a3b-thinking` in `home/lansing/development/pi-coding-agent.nix` (mirrored on the Mac's `~/.pi/agent/settings.json`); `glm-4.6` stays selectable via `/model`. The "GLM-4.6 is the main model" wording in `README.md` / `docs/pi-coding-agent*.md` and the subagent/`models.json` comments is the *intended* steady state, **left in place on purpose** — minimal stopgap, not a model change. Revert `defaultModel` to `glm-4.6` once Cortecs fixes the bug. Full write-up + reproduction: `docs/cortecs-glm46-toolname-bug.md`.
